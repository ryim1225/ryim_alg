...
print(sys.path)
import sort.merge_sort as smt
import draw.paint as dp


class TestMergeSort(unittest.TestCase):
    def setUp(self):
        self.ss = smt.MergeSort()
    ...
    def test_sort_gif(self):
        ...
        csv_file = output_path + "merge_sort_info.csv"
        self.ss.sort_gif(l, csv_file)
        num = dp.make_pictures_of_barchart(csv_file, output_path + "merge_sort_imgs/", "../../draw/seed.png")
        #print(num)
        dp.create_gif([output_path+ 'merge_sort_imgs/'+str(i)+'.png' for i in range(1,num,2)], output_path + "merge_sort.gif", 0.8)


def suite():
    suite = unittest.TestSuite()
    suite.addTest(TestMergeSort("test_sort"))
    suite.addTest(TestMergeSort("test_sort_r"))
    suite.addTest(TestMergeSort("test_sort_gif"))
    return suite
