...
namespace Ryim
{
    ...
    public:
        ...
        typedef arraylist_iterator<_Myt >		iterator;
        typedef arraylist_const_iterator<_Myt > const_iterator;
        ...
        void resize(size_type Newsize)
        {
            ...
            {	// pad as needed
                size_type _Count = Newsize - size();
                if (_Unused_capacity() < _Count)
                {	// need more room, try to get it
                    if (max_size() - size() < _Count)
                        throw illegalIterator("arraylist<T> too long");
                    _reallocate(_Grow_to(size() + _Count));
                }

                std::uninitialized_fill_n(this->m_listLast, Newsize - size(), m_alloc);
        ...
        void resize(size_type Newsize, const value_type& val)
        {
            if (Newsize < size())
                ...
                push_back(end(), Newsize - size(), val);
        }
        ...
        iterator insert(const_iterator where_it, _Iter _First, _Iter _Last)
        ...
        //
        template<class _Iter>
        iterator assign(_Iter _First, _Iter _Last)
    ...
    protected:
        ...
        {	// allocate array with _Capacity elements
            ...
            if (_Capacity == 0)
                ...
                throw Ryim::BaseException("arraylist<T> too long");	// result too long
            else
